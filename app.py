...
def index():
    ...
    if request.method == 'POST' and form.validate():
        ...
        url_list_file_name = '-'.join([url.split("://")[1].split("/")[0],
                                      unique_id])
        url_list_file_name = '.'.join([url_list_file_name, 'txt'])
        print(url_list_file_name)
...
def download_url_file(file_name):
    ...
    return send_file(url_list_file, as_attachment=True,
                     attachment_filename='urls_list.txt')
